...
;(function ($) {
    ...
    // 쿠팡 파트너스 추천상품 (etc/coupang/1016.json, 배포 때 쿠팡 Open API 로 갱신)
    var recommendFactory = {
        url: '/coupang/1016.json',
        products: [],
        cursor: 0,
        load: function (callback) {
            $.getJSON(recommendFactory.url)
                .done(function (data) {
                    recommendFactory.products = recommendFactory.shuffle(
                        $.map((data && data.data) || [], function (item) {
                            return { name: item.productName, image: item.productImage, url: item.productUrl };
                        })
                    );
                    callback(recommendFactory.products.length > 0);
                })
                .fail(function () {
                    callback(false);
                });
        ...
        next: function () {
            var product = recommendFactory.products[recommendFactory.cursor % recommendFactory.products.length];
            recommendFactory.cursor += 1;
            return product;
        }
    };
    ...
    function renderRecommend() {
        ...
        $.each(['first', 'second'], function (index, position) {
            var product = recommendFactory.next();
            $('#recommend-' + position + '-name').html(prodText).append(document.createTextNode(product.name));
            $('.recommend-item-' + position + ' img').attr({ src: product.image, alt: product.name });
            $('.recommend-item-' + position).data('url', product.url);
        });
    }

    // window.outerWidth 는 브라우저 창 바깥 크기라 사이드 패널이 있는 브라우저 등에서 0 이나 작은 값이 나올 수 있다.
    // 실제 페이지 폭으로 판단한다.
    ...
    function customAdFn() {
        recommendFactory.load(function (loaded) {
            if (!loaded) {
                $('#side-recommend').hide();
                return;
            }
            renderRecommend();
            setInterval(renderRecommend, 12000);
        });

        $("#post-footer").css("margin-left", $("#main-content").offset().left);
        $("#side-panel").css("left", $("#main-content").outerWidth() + $("#main-content").offset().left);
            ...
            if ($(".post-header-cover").length < 1) {
                if ($(window).scrollTop() > $(".post-header").offset().top) {
                    if ($(window).scrollTop() < $("#post-footer").offset().top + $(".post-header").outerHeight(true)) {
                        ...
                        $("#side-panel").css("top", "30px");

                    }
                    if ($(window).scrollTop() > $("#post-footer").offset().top - $("#side-panel").outerHeight(true)) {
                        cursor = $("#post-footer").offset().top - $("#side-panel").outerHeight(true) - $(".post-header").outerHeight(true) + 30
                        $("#side-panel").css("position", "absolute");
                        $("#side-panel").css("top", cursor);
                    }
                }
                if ($(window).scrollTop() < $(".post-header").offset().top) {
                    $("#side-panel").css("position", "absolute");
                ...
                if ($(window).scrollTop() > $("#main-content").offset().top - 30) {
                    ...
                    if ($(window).scrollTop() + 30 > $("#post-footer").offset().top - $("#side-panel").outerHeight(
                            true)) {
                        cursor = $("#post-footer").offset().top - $("#side-panel").outerHeight(true) - corverHeight
                        $("#side-panel").css("position", "absolute");
                        $("#side-panel").css("top", cursor);
                    }
                }
    ...
    $(document).ready(function () {
        if ($('.post-bottom-bar').length) {
            ...
            try {
                ...
                // 스크립트 오류가 나도 로딩 화면에 갇히지 않도록 항상 걷어낸다
                $('#loading').remove();
